...
public static class FramePreview
{
    ...
    /// <c>CaptureViewModel.RoiWidth</c>/<c>RoiHeight</c>) means "full frame".
    ///
    /// Used by <see cref="ICameraDevice.SetOutputFormatAsync"/> implementations (e.g.
    /// <c>AsiCameraDevice.ApplyRoiFormat</c>) to work out the actual region to request from the
    /// sensor itself - a genuine hardware ROI, not a post-capture crop (an earlier version of
    /// SolScan's ROI feature only cropped frames after the fact, in software, which left live-view
    /// frame rate unchanged since the camera still transferred every pixel over USB regardless -
    /// confirmed on real ASI678MM hardware; see <see cref="ICameraDevice.SetOutputFormatAsync"/>'s
    /// own doc comment).
    /// </summary>
